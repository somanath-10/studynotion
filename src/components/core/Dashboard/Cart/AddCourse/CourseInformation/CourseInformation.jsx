...
import RequirementsField from "./RequirmentField"
...
export default function CourseInformationForm(){
  ...
  useEffect(() => {
    ...
    if(editCourse){                                                   // if form is in edit mode
  ...
  const onSubmit = async (data) => {    
    console.log("Vkue of editciuraw",editCourse);             
    if(editCourse){
      if(isFormUpdated()) {
        const currentValues = getValues()
    ...
    console.log("Dssdsddf",data?.courseRequirements);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 rounded-md border-[1px] border-richblack-700 bg-richblack-800 p-6" >
      ...
      <button type='submit' className=" px-4 py-2 bg-yellow-100 rounded-md text-richblack-800">
          save changes
        </button>
